...
class SpampedeDisplay {
	...
	public void updateGraphics() {
		...
		// Draw the board
		int yBoardShift = 50;
		for (int col = 0; col < this.theData.getNumColumns(); col++) {
			for (int row = 0; row < this.theData.getNumRows(); row++) {
				this.drawSquare(col * Preferences.CELL_SIZE + Preferences.NUM_CELLS_WIDE,
						row * Preferences.CELL_SIZE + yBoardShift, this.theData.getCellColor(row, col));
			}
		}
