...
class SpampedeData {
	...
	public void moveSnakeForward(BoardCell cell) {
		if (!cell.isSpam()) {
			snakeCells.getFirst().becomeOpen();
	...
	public BoardCell getNextCellFromBFS() {
		...
		while (cellsToSearch.size() != 0) {
			BoardCell removed = cellsToSearch.remove();
			for (BoardCell neighbor : this.getNeighbors(removed)) {
				if (!neighbor.inSearchListAlready() && neighbor.isOpen()) {
					cellsToSearch.add(neighbor);
					neighbor.setAddedToSearchList();
					neighbor.setParent(removed);
					if (neighbor.isSpam()) {
						return this.getFirstCellInPath(neighbor);
					}
	...
	public void reverseSnake() {
		...
		Object[] snakeCellsArray = this.snakeCells.toArray();
		this.snakeCells.clear();
		for (int i = snakeCellsArray.length - 1; i >= 0; i--) {
			this.snakeCells.add((BoardCell) snakeCellsArray[i]);
		}
		...
		if (neck == this.getEastNeighbor()) {
			this.currentMode = SnakeMode.GOING_WEST;
		} else if (neck == this.getWestNeighbor()) {
			this.currentMode = SnakeMode.GOING_EAST;
		} else if (neck == this.getNorthNeighbor()) {
			this.currentMode = SnakeMode.GOING_SOUTH;
		} else {
			this.currentMode = SnakeMode.GOING_NORTH;
		}
